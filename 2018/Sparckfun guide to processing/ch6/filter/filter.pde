...
void setup() {
  ...
  // add filters to the image object
  grin.filter(BLUR, 7);
  scared.filter(GRAY);
  smile.filter(POSTERIZE, 3);
  stair.filter(ERODE);
  unempressed.filter(INVERT);
  zen.filter(THRESHOLD, .8);
}
void draw() {
  background(150);
  ...
  // add the filter function to the end
  // adding filters to the whole collage
  //filter(GRAY);
  //filter(BLUR, 7);
}